<template xmlns:font-size="http://www.w3.org/1999/xhtml">
  ...
  <el-container style="height: 1080px; border: 1px solid #B3C0D1">
    ...
    <el-container>
      ...
      <el-main>
        ...
        <el-col :span="12">
          ...
          <el-button type="primary" @click="dialogFormVisible = true" style="float:right">添加督导</el-button>
    ...
    <!-- 新增督导 -->
                ...
                <template slot="title">
                  <div style="font-size: 25px; color:cornflowerblue; font-weight: bold">添加督导</div>
                  <span>添加督导账号并添加其个人信息</span>
                </template>
                <el-tabs v-model="activeName" >
                  <el-tab-pane label="个人信息" name="first">
                    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
                      ...
                      <el-form-item label="绑定咨询师" prop="bind_username">
                        <el-select
              ...
              <el-dialog :visible.sync="editDialogForm" center width="30%">
                ...
                <template slot="title">
                  <div style="font-size: 25px; color:cornflowerblue; font-weight: bold">修改督导信息</div>
                  <span>修改督导个人信息</span>
                </template>
  ...
  methods:{
    ...
    submitForm(formName) {
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
         addApi({...this.ruleForm,identity:3}).then(res=>{
    ...
    // 搜索确定键=========
    getConsultantList(){
      getListApi({identity:2}).then(res=>{
        if(res.data.status.code===0){
          this.consultantList=res.data.staff_list
        }
      })
    },
   ...
   async confirmEdit() {
      ...
      console.log(this.check_val,33344)
      this.checkSchedule=week.reduce((pre,item)=>{
        pre.push(this.check_val.includes(item)?1:0)
        return pre
      },[]);
       saveApi({...this.toModifyForm,schedule_arr:this.checkSchedule,identity:3}).then(res=>{
         if(res.data.status.code===0){
